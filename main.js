...
global.appInfo = { appVersion: app.getVersion(), appName: "Bitninja Mocap Lite" };

function bitninjaApplyNativeThemePreference() {
    // Stage 2: default to dark mode on first launch, then respect the saved setting.
    // Older SysMocap only wrote this value after the user toggled the switch, which
    // made Bitninja Lite open in light mode even though the Settings UI said dark.
    const savedUseDark = storage.getItem("useDark");
    const shouldUseDark = (typeof savedUseDark === "undefined" || savedUseDark === null)
        ? true
        : !!savedUseDark;
    nativeTheme.themeSource = shouldUseDark ? "dark" : "light";
    if (typeof savedUseDark === "undefined" || savedUseDark === null) {
        storage.setItem("useDark", true);
    }
}
bitninjaApplyNativeThemePreference();

// Stage 5: Linux GPU diagnostics marker. Compare the x11/NVIDIA/MESA launchers when Holistic inference is slow.
// Prevents Chromium from lowering the priority of invisible pages' renderer processes.
// Improve performance while OBS captures the render view.
